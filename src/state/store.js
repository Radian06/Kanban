...
export function moveTask(taskId, newStatus, insertIndex) {
  ...
  // 기존 컬럼에서 제거
  const oldGroup = state.tasks
    .filter(t => t.status === prevStatus && t.id !== task.id)
    .sort((a, b) => a.order - b.order);

  oldGroup.forEach((t, i) => t.order = i);
